...
@section('content')
    <main class="max-w-8xl mx-auto py-10 px-4 sm:px-6 lg:px-8 min-h-screen">
        <h1 class="text-stone-700 text-2xl poppins-semibold">Booking Salon</h1>
        @if ($transactions->isEmpty())
            <p class="mt-5 text-gray-500">You don't have any booking salon history</p>
        @else
                ...
                @foreach ($transactions as $transaction)
                    <div class="bg-white rounded-md border border-gray-300 p-5 flex items-start justify-between">
                        <div class="">
                            ...
                            {{-- <h1 class="poppins-medium text-base">{{ $transaction->service_name }}</h1> --}}

                            <dl class="max-w-md mt-2 text-gray-900 divide-y divide-gray-200">
                                <div class="flex flex-col py-3">
                                    <dt class="mb-1 text-gray-500 md:text-base">Service</dt>
                                    <dd class="text-base font-semibold">
                                        {{ $transaction->service_name }}
                                    </dd>
                                </div>
                                <div class="flex flex-col py-3">
                                    <dt class="mb-1 text-gray-500 md:text-base">Customer Name</dt>
                                    <dd class="text-base font-semibold">
                                ...
                                <div class="flex flex-col py-3">
                                    <dt class="mb-1 text-gray-500 md:text-base">Booking Date</dt>
                                    <dd class="text-base font-semibold">
                                        {{ Carbon\Carbon::parse($transaction->booking_date)->format('l, d F Y') }}
                                    </dd>
                                </div>
                                <div class="flex flex-col py-3">
                                    <dt class="mb-1 text-gray-500 md:text-base">Session</dt>
                                    <dd class="text-base font-semibold capitalize">
                                        {{ $transaction->session }}
                                    </dd>
                                </div>
                                <div class="flex flex-col py-3">
                                    <dt class="mb-1 text-gray-500 md:text-base">Queue Number</dt>
                                    <dd class="text-base font-semibold">
                        ...
                        <div class="">
                            <h1 class="poppins-medium text-base text-stone-800 text-right">
                                {{ format_rupiah($transaction->service_price) }}</h1>
...
@section('script')
    <script>
        ...
        function showDetail() {
            ...
            $.ajax({
                url: "{{ route('transaction.detail', ':id') }}".replace(':id', transactionID),
                type: "GET",
                beforeSend: function() {
